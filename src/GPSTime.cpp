...
void GPSTimeClass::processMessage(const int8_t *msg, uint32_t msgTime)
{
  ...
  { // We're mainly interested in time and date, which RMC contains
    connected = true;

    if (!isMessageValid(msg))
    { // Making sure our message is not corrupt
...
{ // Scan portion --------------------------------------------
  if (serialBaud == 0)
  {
    ...
    if (scanNextSetting)
    {
      ...
      Serial.print("baud(");
      Serial.print(serialBaudRates[curBaudRate], DEC);
      Serial.print(") ");
      Serial.print("config(");
      Serial.print(serialConfigs[curSerialConfig], DEC);
      Serial.print(")\r\n");
